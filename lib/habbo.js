...
export const findMatches = (furnidata, humanName) => {
	...
	const collect = (list, type) =>
		list.flatMap((item) => {
			...
			return r === -1 ? [] : [{ classname: item.classname, type, name: item.name, rank: r }];
		});
...
// Stats come back in the same order as the room/wall items sent, so a separate
// cursor per type lines them back up with names.
export const buildPriceMessage = (shown, data, overflow) => {
	let roomIdx = 0;
	let wallIdx = 0;

	const blocks = shown.map(({ name, type }) => {
		const stats = type === 'room' ? data?.roomItemData?.[roomIdx++] : data?.wallItemData?.[wallIdx++];
